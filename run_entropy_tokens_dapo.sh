...
# Usage: ./run_entropy_tokens_dapo_qwen3.sh [MODEL_PATH] [TRAINING_MODE]
# Example: ./run_entropy_tokens_dapo_qwen3.sh "Qwen/Qwen2.5-3B" "entropy"
# Example: ./run_entropy_tokens_dapo_qwen3.sh "ckpts/hf_models/RLVR-SCE-reward-full-tokens-Qwen2.5-3B-step50" "full"
...
# Accept model path from command line or environment variable
# Default to Qwen/Qwen2.5-3B if not specified
if [ $# -ge 1 ]; then
    MODEL_PATH="$1"
else
...
# Extract model name from path for use in experiment naming
# Handles both "Org/Model" format and "/path/to/model" format
MODEL_NAME=$(basename "${MODEL_PATH}")

# Accept training mode from command line or environment variable
...
if [ $# -ge 2 ]; then
    ...
    TRAINING_MODE=${TRAINING_MODE:-"full"}  # default to full mode
fi

project_name='verl_select_token'
if [ "$TRAINING_MODE" = "full" ]; then
    exp_name="RLVR-with-full-tokens-${MODEL_NAME}-long-data"
    use_entropy_filter=false
    mask_mode='entropy'
    entropy_top_ratio=1.0  # not used in full mode
elif [ "$TRAINING_MODE" = "entropy" ]; then
    exp_name="RLVR-with-only-high-entropy-tokens-${MODEL_NAME}-long-data"
    use_entropy_filter=true
    mask_mode='entropy'
    entropy_top_ratio=0.2
elif [ "$TRAINING_MODE" = "probability" ]; then
    exp_name="RLVR-with-probability-based-mask03-${MODEL_NAME}-long-data"
    use_entropy_filter=true
    mask_mode='probability'
    max_prob_threshold=0.3
elif [ "$TRAINING_MODE" = "entropy-probability" ]; then
    exp_name="RLVR-with-entropy02-probability-mask-${MODEL_NAME}-long-data"
    use_entropy_filter=true
    mask_mode='entropy-probability'
    entropy_top_ratio=0.2  # lower ratio for combined mode
else
...
# MODEL_PATH is set above from command line argument or environment variable
CKPTS_DIR=${CKPTS_DIR:-"${PROJECT_ROOT}/ckpts/${project_name}/${exp_name}"}
# TRAIN_FILE=${TRAIN_FILE:-"${RAY_DATA_HOME}/data/train.parquet"}
# VAL_AIME24_FILE=${VAL_AIME_FILE:-"${RAY_DATA_HOME}/data/aime_2024.parquet"}
# VAL_AIME25_FILE=${VAL_AIME_FILE:-"${RAY_DATA_HOME}/data/aime_2025.parquet"}
# VAL_MATH500_FILE=${VAL_MATH500_FILE:-"${RAY_DATA_HOME}/data/test.parquet"}
# VAL_FILES=${VAL_FILES:-"['${VAL_AIME24_FILE}', '${VAL_AIME25_FILE}', '${VAL_MATH500_FILE}']"}
